...
def write_counter_top(ws, start_row, details, sizes, colors, sink_counts):
    ...
    for i, color in enumerate(colors):
        max_col_width[i+1] = len(color) + 10
        ws.cell(row, i+2).value = color
        ws.cell(row, i + 2).font = Font(bold=True, size="20")
        ws.cell(row, i+2).alignment = Alignment(horizontal="center")
        ws.cell(row, i+2).fill = PatternFill(start_color="3FDD38", fill_type = "solid")
    ...
    ws.cell(row, 1).font = Font(bold=True, size="15")
    ws.cell(row, 1).border = border_style
    for i, color in enumerate(colors):
        if total[i]:
            ws.cell(row, i+2).value = total[i]
            ws.cell(row, i+2).alignment = Alignment(horizontal="center")
            ws.cell(row, i+2).font = Font(bold=True, size="13")
    ...
    ws.cell(row, 2).font = Font(bold=True, size="13")
