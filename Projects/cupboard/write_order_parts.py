...
def write_material_items(work_sheets, start_row, items):
    ...
    for ws in work_sheets:
        ...
        ws.cell(row, 1).fill = PatternFill(start_color="3FDD38", fill_type="solid")
        ws.cell(row, 1).font = Font(bold=True , size="20")
        ws.cell(row, 2).value = items[0][1]
        ws.cell(row, 2).font = Font(bold=True ,size="20")
        ws.cell(row, 2).fill = PatternFill(start_color="3FDD38", fill_type="solid")
    ...
    for ws in work_sheets:
        ...
        if ws.title == SHEET_COLOR_COUNT:
            for i, color in enumerate(colors):
                ...
                ws.cell(row, i+3).fill = PatternFill(start_color="ffff00", fill_type="solid")
...
def write_part_names(work_sheets, start_row, item, details, sizes, styles, colors):
    ...
    '''
    #excel cell styles
    border_style = Border(top = Side(style='double' , color='3C40E1'), bottom = Side(style='thin' , color='3C40E1'))
    ...
    for ws in work_sheets:
        ws.cell(row,1).value = item
        ws.cell(row, 1).font = Font(bold=True)
        ws.cell(row, 1).fill = PatternFill(start_color="ffff00", fill_type="solid")
